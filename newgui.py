...
class ViewPollsWindow(QDialog):
    ...
    def init_ui(self):
        ...
        try:
            with self.parent.connection.cursor() as cursor:
                # Retrieve all active polls from the POLL table
                poll_query = "SELECT * FROM POLL WHERE START_DATE <= %s AND END_DATE >= %s"
                cursor.execute(poll_query, (current_datetime, current_datetime))
...
class VoteItemWindow(QDialog):
    ...
    def init_ui(self):
        ...
        try:
            with self.parent.connection.cursor() as cursor:
                # Retrieve the end date of the poll
                cursor.execute("SELECT END_DATE FROM POLL WHERE POLL_ID = %s", (self.poll_id,))
                poll = cursor.fetchone()
                self.poll_end_date = datetime.strptime(poll['END_DATE'], '%Y-%m-%d %H:%M:%S')
